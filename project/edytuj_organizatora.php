...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $id = $_POST['organizator_id'];

    $sql = "UPDATE organizatorzy SET
    nazwa_organizatora = '{$nazwa}', adres = '{$adres}',mail = '{$mail}',telefon = '{$telefon}'
                                WHERE organizatorzy_id = '{$id}' ";
...
<body>
  ...
  <body>
      <div class="wrapper">
       <form method="post">
           <?php
           $id = $_GET['organizator_id'];
           $sql = "SELECT * FROM organizatorzy WHERE organizatorzy_id = '{$id}'";
        ...
        <div class="form-group">
            ...
            value='<?php echo $row['nazwa_organizatora'];?>' placeholder="Nazwa organizatora">
        </div>

        <div class="form-group">
            <label>Adres firmy</label>
            <input type="text" name="adres" class="form-control"
            value='<?php echo $row['adres'];?>' placeholder="Adres firmy">
        </div>

        <div class="form-group">
            <label>Email</label>
            <input type="email" name="mail" class="form-control"
            value='<?php echo $row['mail'];?>' placeholder="Email">
        </div>

        <div class="form-group">
            <label>Telefon</label>
            <input type="number" name="telefon" class="form-control"
            value='<?php echo $row['telefon'];?>' placeholder="Telefon">
        </div>

        <input type="hidden" name="organizator_id" value="<?php echo $row['organizatorzy_id']; ?>">
